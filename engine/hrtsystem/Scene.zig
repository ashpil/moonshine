...
pub fn fromGlbExr(vc: *const VulkanContext, vk_allocator: *VkAllocator, allocator: std.mem.Allocator, image_manager: *ImageManager, commands: *Commands, glb_filepath: []const u8, skybox_filepath: []const u8, extent: vk.Extent2D, inspection: bool) !Self {
    ...
    _ = try camera.appendSensor(vc, vk_allocator, allocator, image_manager, extent);

    var world = try World.fromGlb(vc, vk_allocator, allocator, commands, gltf, image_manager, inspection);
    ...
    errdefer background.destroy(vc, allocator);
    {
        ...
        try background.addBackground(vc, vk_allocator, allocator, image_manager, commands, skybox_image, "exr");
    }
