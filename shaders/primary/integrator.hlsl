    ...
    float3 incomingRadiance(RayDesc initialRay, inout Rng rng) {
        ...
        for (Intersection its = Intersection::find(ray); its.hit(); its = Intersection::find(ray)) {
            ...
            for (uint directCount = 0; directCount < env_samples_per_bounce; directCount++) {
                accumulatedColor += throughput * estimateDirectMIS(frame, EnvMap::create(), material, outgoing, attrs.position, attrs.normal, rng) / env_samples_per_bounce;
            }
            ...
            for (uint directCount = 0; directCount < mesh_samples_per_bounce; directCount++) {
                ...
                accumulatedColor += throughput * estimateDirect(frame, MeshLights::create(), material, outgoing, attrs.position, attrs.normal, rng) / mesh_samples_per_bounce;
            }
