...
#include "../primary/math.hlsl"
#include "../primary/reflection_frame.hlsl"
#include "../primary/random.hlsl"
#include "../primary/bindings.hlsl"
...
struct StandardPBR : Material {
    ...
    static StandardPBR create(uint materialIndex, float2 texcoords) {
        MaterialInput input = dMaterials[NonUniformResourceIndex(materialIndex)];

        float roughness = max(dMaterialTextures[NonUniformResourceIndex(input.roughness)].SampleLevel(dTextureSampler, texcoords, 0).r, 0.004);

        StandardPBR material;
        material.color = dMaterialTextures[NonUniformResourceIndex(input.color)].SampleLevel(dTextureSampler, texcoords, 0).rgb;
        material.metalness = dMaterialTextures[NonUniformResourceIndex(input.metalness)].SampleLevel(dTextureSampler, texcoords, 0).r;
        material.ior = input.ior;
