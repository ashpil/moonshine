...
pub fn fromGltfExr(vc: *const VulkanContext, allocator: std.mem.Allocator, io: std.Io, encoder: *Encoder, gltf_filepath: []const u8, skybox_filepath: []const u8, extent: vk.Extent2D, chromaticities: engine.color.Chromaticities) !Self {
    ...
    _ = try camera.appendSensor(vc, allocator, extent, chromaticities);
...
pub fn pushDescriptors(self: *const Self, camera: u32, sensor: u32, background: u32) engine.hrtsystem.pipeline.Render.PushSetBindings {
    _ = camera;
...
pub fn pushConstants(self: *const Self, camera: u32, sensor: u32, background: u32, seed: u32) engine.hrtsystem.pipeline.Render.PushConstants {
    return engine.hrtsystem.pipeline.Render.PushConstants {
        ...
        .xyz_to_dst_chromaticities = self.camera.sensors.items[sensor].chromaticities.fromXYZ().floatCast(f32),
