...
pub fn main() !void {
    ...
    // actual ray tracing
    {
        ...
        for (0..config.spp) |sample_count| {
            // push our stuff
            pipeline.recordPushConstants(encoder.buffer, .{ .lens = scene.camera.lenses.items[0], .aspect_ratio = scene.camera.sensors.items[0].aspectRatio(), .sample_count = scene.camera.sensors.items[0].sample_count });
